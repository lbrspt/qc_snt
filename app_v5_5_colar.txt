...
def render_dashboard():
    ...
    <div class="main-header">
        <div style="display:flex;justify-content:space-between;align-items:center;">
            <div>
                <div class="brand-line">SNT · CMT</div>
                <h1>🏭 SNT CMT</h1>
                <p>{t('h_sub')}</p>
            </div>
    ...
    try:
        ...
        # v5: alertas só sobre refs com atividade (stock, encomendas ou necessidade) —
        # refs paradas a 0 deixam de gerar ruído de "stock baixo" / "sem cor"
        alerts_df = stock_df[(stock_df['total_stock'] > 0) | (stock_df['a_chegar'] > 0) | (stock_df['necessidade'] > 0)]
        critical = alerts_df[alerts_df['status'].str.contains(t('st_crit'), regex=False)]
        warning = alerts_df[alerts_df['status'].str.contains(t('st_low'), regex=False)]
        risk = alerts_df[alerts_df['status'].str.contains(t('st_risk'), regex=False)]
        ok = alerts_df[alerts_df['status'].str.contains('ok')]
