...
'pt': {
 ...
 'c_edit_reg_sub': 'corrige peças/metros de um registo ou remove linhas — o real médio do mapa é recalculado automaticamente. lançamentos novos fazem-se SEMPRE em 👕 Produção → ⚡ Modo Live.',
...
'en': {
 ...
 'c_edit_reg_sub': 'edit pieces/metres of a record or remove rows — the map real average recalculates automatically. New entries are ALWAYS made in 👕 Production → ⚡ Live Mode.',
...
def link_rolls_for_cut(po, ref, color, conf, target_m):
    ...
    for r in query_to_df(f"""SELECT token, metres, color, warehouse, lot, date_received FROM fabric_rolls
                             WHERE ref_code = ? {color_sql} AND status = 'IN_PROCESS' AND warehouse = ?
                               AND (po_garment IS NULL OR po_garment = '')
                             ORDER BY date_received, token""", (*base_params, conf)).to_dict('records'):
        if gap <= 0.05:
            break
        ...
        if take >= m_roll - 0.001:
            ...
            log_movement('LINK', new_tok, conf, conf, ref, take, po,
...
# ===================== UI: DASHBOARD =====================
def render_stock():
    """📦 Stock (v7) — com 🚢 A Chegar como separador (era menu próprio)."""
...
def render_today():
    ...
    <div class="main-header">
        <div style="display:flex;justify-content:space-between;align-items:center;">
            <div>
                <div class="brand-line">SNT · CMT · ULTRA</div>
                <h1>{t('m_today')}</h1>
                <p>{t('h_sub')}</p>
            </div>
...
def render_tools():
    ...
    tool = st.radio(t('m_system'), [t('m_movement'), t('tr_title'), t('ex_title'), t('m_intg')],
...
def main():
    ...
    <div style="position:fixed;bottom:20px;left:20px;right:20px;">
        <div style="border-top:1px solid var(--line);padding-top:12px;color:var(--faint);font-size:11px;text-align:center;">
            v7.1 | {t('sb_data')}<br>{datetime.now().strftime('%Y-%m-%d')}<br>
